...
class DashboardController extends Controller
{
    ...
    // private function singkat(string $nama): string
    // {
    //     return trim(preg_replace('/\s+/', ' ',
    //         str_replace(['Puskesmas ', 'UPTD ', 'UPT PKM ', 'UPT '], '', $nama)
    //     ));
    // }
 
    // private function warna(float $pct): string
    // {
    //     return match (true) {
    //         $pct >= 70 => 'rgba(34,197,94,.80)',
    //         $pct >= 50 => 'rgba(245,158,11,.80)',
    //         $pct >= 30 => 'rgba(249,115,22,.80)',
    //         default    => 'rgba(239,68,68,.80)',
    //     };
    // }
    ...
    //     $allIndikators = NewIndikator::where('status_aktif', true)->orderBy('id_indikator')->get();
    ...
    //         ->select(
    //             'dpd.id_indikator',
    //             DB::raw('SUM(dpd.jumlah_sasaran) AS sasaran_total'),
    //             DB::raw('SUM(dpd.jumlah_capaian) AS capaian_total'),
    ...
    //     $rekapIndikator = $allIndikators->map(function($ind) use ($detailAgg) {
    //         $agg = $detailAgg->get($ind->id_indikator);
    //         return (object) [
    //             'nama_indikator'  => $ind->nama_indikator,
    //             'target_nasional' => 70, // Nilai default nasional
    //             'persentase'      => $agg ? (float) $agg->persentase : 0,
    //         ];
    //     });
    ...
    //         $pkData = $indPerPkm->where('id_puskesmas', $row->id_puskesmas);
    //         foreach ($pkData as $d) {
    //             $row->{"ind{$d->id_indikator}_pct"} = $d->pct;
    //         }
    //     }

    //     // ── Grafik Batang ─────────────────────────────────────────────────────
    //     $grafikLabels = $rekapData->pluck('nama_puskesmas')->values()->toArray();
    //     $grafikData   = $rekapData->pluck('persentase_phbs')->values()->toArray();
 
    //     $availableTahun = collect(range(date('Y') - 2, date('Y'))); // Sederhanakan drop down tahun jika belum banyak data
    ...
    //         'grafikLabels', 'grafikData', 'tahun', 'bulan', 'id_puskesmas', 'puskesmasList', 'availableTahun'
    //     ));
    // }
